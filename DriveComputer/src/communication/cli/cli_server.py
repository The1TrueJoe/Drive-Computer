...
class CLI_Server:
    ...
    def __init__(self, establish_port=42069, command_port=69, log_port=420, response_port=777):
        ...
        self.cmd_map = {
            ...
            "disable": main.drive.disable


        }

        self.cmd_1_param_map = {
    ...
    def establishListen(self):
        ...
        while not self.kill and not self.connection_established:
            ...
            # If message received
            if data == "Bruh, I want to command you!":
    ...
    def commandListener(self):
        ...
        while self.connection_established and not self.kill:
            ...
            if clientAddress == self.connected_ip:
                # Commands
                if data in self.cmd_map.keys():
                    self.current_response_message = self.response = self.cmd_map[data]()
                    
                elif data.split()[0] in self.cmd_1_param_map.keys():
                    self.current_response_message = self.cmd_1_param_map[data.split()[0]](data.split[1])
